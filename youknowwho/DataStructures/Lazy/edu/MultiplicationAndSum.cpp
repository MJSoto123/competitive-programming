...
struct SegmentTree{
    ...
    void build(int node, int l, int r){
        ...
        t[node] = add(t[2 * node + 1], t[2 * node + 2]);
    }
    
    SegmentTree(int nn) : n(nn), t(4 * nn + 10, 1), lz(4 * nn + 10, 1) {
        build(0, 0, n -1);
    }
    ...
    void push(int node, int l, int r){
        if(lz[node] == 1) return;
        if(l != r){
            lz[2 * node + 1] = mul(lz[2 * node + 1], lz[node]);
            lz[2 * node + 2] = mul(lz[2 * node + 2], lz[node]);
        }
        t[node] = mul(t[node], lz[node]);
        lz[node] = 1;
    }
