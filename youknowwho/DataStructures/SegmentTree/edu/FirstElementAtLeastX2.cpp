...
struct SegmentTree{
    ...
    int treeDescend(int node, int l, int r, int lq, int val){
        if(r < lq) return -1;
        ...
        if(lq <= mid) ans = treeDescend(2 * node + 1, l, mid, lq, val);
        if(ans == -1) ans = treeDescend(2 * node + 2, mid + 1, r, lq, val);
        return ans;
    }
    int treeDescend(int lq, int val){ return treeDescend(0, 0, n - 1, lq, val); }
};
...
int main(){
    ...
    rep(i, 0, q){
        ...
        if(op == 1){
            ...
            int val, lq; cin >> val >> lq;
            cout << st.treeDescend(lq, val) << "\n";
        }
    }
}