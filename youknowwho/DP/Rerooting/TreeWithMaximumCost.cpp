...
int a[N];
elem E() { return {0, 0}; }
elem take_vertex(elem DP, int v) { return {DP.sum + a[v], DP.ans}; }
elem up_root(elem DP, int e) { return {DP.sum, DP.ans + DP.sum}; }
elem merge(elem u, elem v) { return {u.sum + v.sum, u.ans + v.ans}; }
...
int main(){
	...
	ll ans = 0;
	rep(i, 0, n) ans = max(ans, ret[i].ans);
	cout << ans << "\n";
}