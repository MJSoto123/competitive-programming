...
elem up_root(elem DP, int e) { return { DP.ans + (e % 2 == 0 ? 0 : 1)}; }
elem merge(elem u, elem v) { return {u.ans + v.ans}; }
...
int main(){
    ...
    int mn = 1e9;
    rep(i, 0, n) mn = min(mn, ret[i].ans);

    cout << mn << "\n";
    rep(i, 0, n) if(mn == ret[i].ans) cout << i + 1 << " ";
    cout << "\n";
}